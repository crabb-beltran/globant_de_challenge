...
class TestDepartmentsIngestion:
    ...
    def test_clean_batch_inserts_all(self, db_session):
        records = [
            Departments(id=1, department="Sales"),
            Departments(id=2, department="Marketing"),
        ]
    ...
    def test_duplicate_against_existing_db_row_is_rejected(
        self, db_session, seed_department
    ):
        ...
        assert (
            db_session.query(Department).filter_by(id=1).one().department
            == "Engineering"
        )

    def test_duplicate_within_same_batch_only_first_inserted(self, db_session):
        records = [
            Departments(id=5, department="Finance"),
            Departments(id=5, department="Finance (dup)"),
        ]
    ...
    def test_partial_batch_isolates_failure_207_semantics(
        self, db_session, seed_department
    ):
        """Core contract of Fase 6: one invalid record must not block valid ones."""
        records = [
            Departments(
                id=1, department="dup, should reject"
            ),  # duplicate vs seed_department
        ...
        assert {r["reason_code"] for r in result["rejected_records"]} == {
            "DUPLICATE_ID"
        }
...
class TestEmployeesIngestion:
    ...
    def test_valid_employee_with_existing_fks_inserts(
        self, db_session, seed_department, seed_job
    ):
    ...
    def test_nonexistent_department_fk_rejected_with_correct_code(
        self, db_session, seed_job
    ):
        record = HiredEmployeeSchema(
            id=101,
            name="Carlos Ruiz",
            hire_datetime="2021-03-15T10:00:00Z",
            department_id=999,
            job_id=1,  # department 999 does not exist
        )
    ...
    def test_nonexistent_job_fk_rejected_with_correct_code(
        self, db_session, seed_department
    ):
        record = HiredEmployeeSchema(
            id=102,
    ...
    def test_mixed_batch_valid_and_fk_violation_isolated(
        self, db_session, seed_department, seed_job
    ):
        ...
        records = [
            HiredEmployeeSchema(
                id=200,
                name="Valid Employee",
                hire_datetime="2021-01-10T08:00:00Z",
                department_id=1,
                job_id=1,
            ),
            HiredEmployeeSchema(
                id=201,
                name="Bad FK Employee",
                hire_datetime="2021-01-11T08:00:00Z",
                department_id=999,
                job_id=1,
            ),
        ]
